...
class UserCollecting extends React.Component<IProps, IState> {
  state = {
    list: [],
    count: 0,
    hasMore: true,
  ...
  componentDidMount() {
    this.queryCollectingPhotos();
  }

  queryCollectingPhotos = async (loadMore?: boolean) => {
    ...
    const res = await photoApi.getCollectingPhotos({
      user_id: userId,
      page,
      per_page
    });
  ...
  loadMore = async () => {
    ...
    this.queryCollectingPhotos(true);
  };
