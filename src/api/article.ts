...
export const articleApi = {
  ...
  getArticleState: (params: { article_id: string }) => {
    return Request({ url: `/articles/${params.article_id}/state`, method: 'get' });
  },

  // 下一组
  getNextArticle: (params: { article_id: string }) => {
    return Request({ url: `/articles/${params.article_id}/next`, method: 'get' });
  },
  ...
  // 收藏文章、取消收藏
  collectArticle: (params: { article_id: string, collecting_state: boolean }) => {
    const method = params.collecting_state ? 'delete' : 'put';
    return Request({ url: `/articles/collecting/${params.article_id}`, method });
  },

  // 分享文章
  shareArticle: (params: { article_id: string }) => {
    return Request({ url: `/articles/shareing/${params.article_id}`, method: 'put' });
  }
